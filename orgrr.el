...
;; a new search - even in a search buffer
;;
;; 1.1.1.
...
If SUPER is non-nil, the results are directly inserted there."
  (interactive "P")
  (let* ((call-with-arg nil)
	 (filename (or filename (if (equal major-mode 'dired-mode)
				    default-directory
				  (buffer-file-name))))
    ...
    (orgrr-get-all-meta)
    (when (or (not (string-match-p "backlinks for *" (buffer-name (current-buffer))))
	      super)
...
;; main part preparing the findings for the orgrr-show-backlinks buffer
	...
	(insert "(q=kill this buffer, n=next heading, p=previous heading, <=goto first heading, >=goto last heading, return=open link, tab=next link, space=scroll)\n\n")
		...
		(when (and (stringp entry)
		  ...
		  (let ((key entry)
			...
			(let* ((line-number (match-string 1 value))
			       ...
			       (snippet (string-trim-left (string-trim-left snippet "*"))))
			  (insert (concat "\*\*\* \[\[file:" full-filename "::" line-number "\]" "\[" result "\]\]:\n\n"  snippet "\n\n"))))))))))
	  (when (and (string-match-p "backlinks for *" (buffer-name (current-buffer)))
		     (not super))
	    (orgrr-close-buffer))))
...
variable orgrr-window-management."
  ...
  (when (not (string-match-p "sequence for *" (buffer-name (current-buffer))))
	...
	(let ((inhibit-read-only t))
	  (setq buffer-read-only nil)
          (erase-buffer)
	  (insert (concat "Sequence for "(orgrr-return-fullzettel-linked-head selection-zettel) "\n\n"))
	  (insert "(q=kill this buffer, n=next heading, p=previous heading, <=goto first heading, >=goto last heading, return=open link, tab=next link, space=scroll)\n\n")
...
patient."
   ...
   (let ((call-with-arg nil))
	 ...
	 (with-current-buffer (get-buffer-create relatednotes-buffer)
	   ...
	   (if (= 1 related-notes)
	       (insert (concat (number-to-string related-notes) " related note for " (orgrr-return-fullnote-linked-starred title) "\n\n"))
	     (insert (concat (number-to-string related-notes) " related notes for " (orgrr-return-fullnote-linked-starred title) "\n\n")))
	   (insert "(q=kill this buffer, n=next heading, p=previous heading, <=goto first heading, >=goto last heading, return=open link, tab=next link, space=scroll)\n\n")
   ...
   (let ((call-with-arg nil))
	 ...
	 (when current-zettel
	   (with-current-buffer (get-buffer-create multiverse-buffer)
	     ...
	     (insert (concat "Multiverse for " (orgrr-return-fullzettel-linked-starred selection-zettel) "\n\n"))
	     (insert "(q=kill this buffer, n=next line, p=previous line, <=goto first heading, >=goto last heading, return=open link, tab=next link, space=scroll)\n\n")
	     (if (not (string-equal parent-zettel ""))
	     ...
	     (when sequence-p
	       (insert "\n"))
	     (if (= 1 related-notes)
		 (insert (concat "* " (number-to-string related-notes) " related note:\n\n"))
	       (insert (concat "* " (number-to-string related-notes) " relates notes:\n\n")))
	     (maphash (lambda (key value)
			(push (cons value key) sorted-values))
		      orgrr-filename-mentions)
	     (setq sorted-values (sort sorted-values (lambda (a b) (> (car a) (car b)))))
	...
	(let*  ((zettel-filename (gethash starting-point orgrr-title-filename))
	  ...
	  (with-current-buffer (get-buffer-create draft-buffer)
	    (let ((inhibit-read-only t))
	      (setq buffer-read-only nil)
              (erase-buffer)
	      (insert (concat "Compiled sequence for " (orgrr-return-fullzettel-linked-starred selection-zettel) " - " (orgrr-return-fullzettel-linked-starred end-point) "\n\n"))
	      (insert "(q=kill this buffer, n=next heading, p=previous heading, <=goto first heading, >=goto last heading, return=open link, tab=next link, space=scroll)\n\n")
	      (dolist (element orgrr-zettel-list)
		(let* ((last-char (substring selection-zettel -1))
		       (is-last-char-num (string-match-p "[0-9]" last-char))
...
(defun orgrr-super-compile-sequence (arg)
  "Creates a temporary buffer with all notes between two selected zettels
(e.g. between two notes with values for zettel). Sequencing works similar
as in orgrr-show-sequence.
...
If called with C-u the buffer is created without headlines."
  ...
  (let ((call-with-arg nil))
    (when (equal arg '(4))
      (setq call-with-arg 1))
    (when (not (string-match-p "*super compiled sequence*" (buffer-name (current-buffer))))
      (let* ((current-zettel (orgrr-read-current-zettel))
	     (orgrr-selection-list (orgrr-prepare-zettel-selection-list))
	     (orgrr-selection-list-completion (orgrr-presorted-completion-table orgrr-selection-list))
	     (orgrr-zettel-list (hash-table-values orgrr-filename-zettel))
	     ...
	     (end-point)
	     (end-flag)
	     (draft-buffer "*super compiled sequence*"))
	(if current-zettel
	    (setq starting-point (completing-read "Select starting point: " orgrr-selection-list-completion nil nil (orgrr-return-fullzettel current-zettel)))
	  (setq starting-point (completing-read "Select starting point: " orgrr-selection-list-completion)))
	(if (string-match "^\\[\\(.*?\\)\\]" starting-point)
	...
	(let*  ((zettel-filename (gethash starting-point orgrr-title-filename))
	  ...
	  (with-current-buffer (get-buffer-create draft-buffer)
	    (let ((inhibit-read-only t))
	      (setq buffer-read-only nil)
              (erase-buffer)
	      (insert (concat "Super compiled sequence for " (orgrr-return-fullzettel-linked-starred selection-zettel) " - " (orgrr-return-fullzettel-linked-starred end-point) "\n\n"))
	      (insert "(q=kill this buffer, n=next heading, p=previous heading, <=goto first heading, >=goto last heading, return=open link, tab=next link, space=scroll)\n\n")
	      (dolist (element orgrr-zettel-list)
		(let* ((last-char (substring selection-zettel -1))
		       (filename (gethash element orgrr-zettel-filename))
		       (is-last-char-num (string-match-p "[0-9]" last-char))
		       (regex (if (and is-last-char-num
				       (not (string-equal element selection-zettel)))
				  (concat "^" selection-zettel "[a-zA-Z]")
...
;;Increase the Orgmode headings of the original note by one
		    (let* ((zettel-content (orgrr-return-fullzettel-content element))
			   (zettel-content (replace-regexp-in-string "^\\(*\\)" "\\1*" zettel-content)))
		    (insert (concat zettel-content "\n\n")))
		    (orgrr-show-backlinks '(4) filename t))
		  (when (equal element end-point)
		      (setq end-flag t)))))
;;Starting here it is only window-management
	      (orgrr-open-buffer draft-buffer)
...
containers will be searched. Regex don't need to be escaped."
 (interactive "P")
  (let ((call-with-arg nil))
	...
	(if call-with-arg
	    (if (= hits 1)
		(insert (concat "Global search returned one finding for *" search "*\n\n"))
	      (insert (concat "Global search returned " (number-to-string hits) " findings for *" search "*\n\n")))
	  (if (= hits 1)
	      	(insert (concat "Local search returned one finding for *" search "*\n\n"))
	      (insert (concat "Local search returned " (number-to-string hits) " findings for *" search "*\n\n"))))
	(insert "(q=kill this buffer, n=next heading, p=previous heading, <=goto first heading, >=goto last heading, return=open link, tab=next link, space=scroll)\n\n")
